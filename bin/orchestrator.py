...
OVERLAP_SEC = 0.0
SAMPLE_RATE = 16000
ENGINE_BIN = str(Path(__file__).resolve().parents[1] / "bin" / "run-infer-engine")
FA_BIN = str(Path(__file__).resolve().parents[1] / "bin" / "run-forcealigner")
...
class InferDaemon:
    ...
    def __init__(self, model_dir: str, mel_path: str, stderr=None):
        self.proc = subprocess.Popen(
            ...
            env={
                **os.environ,
                "GLMASR_VRAM_UTIL": os.environ.get("GLMASR_VRAM_UTIL", "0.9"),
                "GLMASR_MAX_SEQS": os.environ.get("GLMASR_MAX_SEQS", "256"),
                "GLMASR_MAX_BATCHED_TOKENS": os.environ.get("GLMASR_MAX_BATCHED_TOKENS", "10000"),
                "GLMASR_MAX_NEW_TOKENS": os.environ.get("GLMASR_MAX_NEW_TOKENS", "256"),
                "GLMASR_ENCODER_FA_PIPELINED": os.environ.get("GLMASR_ENCODER_FA_PIPELINED", "1"),
                "GLMASR_ENCODER_FA_VLLM": os.environ.get("GLMASR_ENCODER_FA_VLLM", "1"),
                "GLMASR_ENCODER_FA_LONG_MIN_SEQ": os.environ.get("GLMASR_ENCODER_FA_LONG_MIN_SEQ", "1"),
                "GLMASR_ENCODER_FA_MIN_SEQ": os.environ.get("GLMASR_ENCODER_FA_MIN_SEQ", "1"),
            },
        )
...
def copy_asr_only_output(
    ...
    if output_path is None:
        return None
    output_path.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(transcript_tsv, output_path)
    if logger is not None:
        logger.log(f"[asr] copied transcript TSV to {output_path}")
    ...
    for key, group_task_dirs in groups.items():
        ...
        try:
            ...
            for info in infos:
                ...
                asr_output = copy_asr_only_output(
                    paths.transcript_tsv,
                    paths.output_path if args.asr_only else None,
                    info["logger"],
                )
                result = {
                    "transcript_tsv": str(paths.transcript_tsv),
                    "output": asr_output if args.asr_only else str(paths.output_path),
                    "chunks": len(segments),
                    "asr_chunks": len(texts),
                    "asr_infer_sec": info["infer_ms"] / 1000.0,
