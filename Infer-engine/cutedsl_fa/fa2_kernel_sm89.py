"""SM89/Ada varlen FA2 export.

This is a distribution-oriented copy of fa2_kernel.py for RTX 40-series GPUs.
It writes artifacts into cutedsl_fa/sm89_build so it never overwrites the SM120
objects used by the local RTX 50-series path.

Distribution note: do not build this with the `nvidia-cutlass-dsl[cu13]` extra
when targeting common RTX 40-series CUDA 12 driver stacks. The `cu13` backend
emits PTX 9.1 / CUDA 13.1 cubins. Build with `nvidia-cutlass-dsl` +
`nvidia-cutlass-dsl-libs-base` only; that backend emits CUDA 12.9 artifacts
which have been validated on an RTX 4090 with driver 570.x.
"""
...
from cutlass.base_dsl import version_info as cutlass_dsl_version
...
OUT_DIR = Path(__file__).resolve().parent / "sm89_build"
OUT_DIR.mkdir(exist_ok=True)
BUILD_ARCH = os.environ.get("GLMASR_CUTEDSL_SM89_ARCH", "sm_89")
DUMP_DIR = os.environ.get("GLMASR_CUTEDSL_SM89_DUMP_DIR", "")
EXTRA_OPTIONS = os.environ.get("GLMASR_CUTEDSL_SM89_EXTRA_OPTIONS", "")


def compile_options() -> str:
    opts = f"--gpu-arch {BUILD_ARCH} --enable-tvm-ffi"
    if DUMP_DIR:
...
def warn_backend_compatibility() -> None:
    cuda_version = cutlass_dsl_version.CUDA_VERSION
    print(
        f"[fa2-sm89] CuTe DSL backend CUDA_VERSION={cuda_version.major}.{cuda_version.minor}; "
        f"target={BUILD_ARCH}; out={OUT_DIR}"
    )
    if cuda_version.major >= 13:
        print(
            "[fa2-sm89] WARNING: this CuTe DSL wheel emits CUDA 13.x / PTX 9.x artifacts. "
            "They require a CUDA 13-capable NVIDIA driver and will not load on driver 570.x "
            "(CUDA 12.8). Use a CUDA 12.x CuTe DSL backend or a non-CuTe CUDA12 kernel for "
            "570-compatible distribution."
        )
    else:
        print(
            "[fa2-sm89] CUDA 12.x CuTe DSL backend detected. This is the expected path for "
            "RTX 40-series distribution on driver 570.x."
        )
